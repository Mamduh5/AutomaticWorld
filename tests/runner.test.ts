...
describe('continuous runner',()=>{
  ...
  it('finishes an in-flight cycle before graceful abort and persists shutdown',async()=>{let cognitionStarted!:()=>void;const started=new Promise<void>((resolve)=>{cognitionStarted=resolve;}),provider:CognitionProvider={async think(){cognitionStarted();await new Promise((resolve)=>setTimeout(resolve,20));return{thoughtSummary:'wait',selectedAction:{type:'WAIT',ticks:1}};}};const e=await engine(provider),controller=new AbortController(),running=new ContinuousWorldRunner(e).run({tickMs:100,maxTicks:10,signal:controller.signal});await started;controller.abort();const result=await running;expect(result.reason).toBe('shutdown signal');expect(result.ticks).toBe(1);expect(e.repo.getWorld()?.currentTick).toBe(1);expect(e.repo.listEvents().some((event)=>event.type==='RUNNER_STOPPED')).toBe(true);e.close();});
});
