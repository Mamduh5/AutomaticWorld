...
describe('continuous runner',()=>{
  it('honors a maximum tick count and releases its database lease',async()=>{const e=await engine();const result=await new ContinuousWorldRunner(e).run({tickMs:0,maxTicks:2});expect(result).toEqual({ticks:2,reason:'maximum ticks reached'});expect(e.repo.getWorld()?.currentTick).toBe(2);expect(e.repo.acquireRunnerLease('later',1_000)).toBe(true);e.close();});
