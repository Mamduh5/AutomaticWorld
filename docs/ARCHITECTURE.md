...
An observation contains only self state, relevant event summaries, addressed messages, separately listed private/shared artifacts, recent self-owned executions, relevant self-owned memories, resources, structured kernel capability descriptions, and accessible userland-tool summaries. Cognition has no repository, filesystem, environment, gateway, or Docker access. Tool discovery changes observations, never the foundational identity prompt.
