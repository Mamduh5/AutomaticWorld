...
Live readiness checks provider connectivity with an authenticated model-metadata GET rather than an inference request. Reports persist only the configured provider/model identifiers, bounded usage counters, latency, and sanitized success/failure state. Unexpected provider exception text is replaced before it reaches SQLite, so URLs or credentials echoed by a client cannot become world history. The readiness doctor scans world-data for exact configured credential values and reports only a count, never the values.
