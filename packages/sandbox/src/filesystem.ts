...
export class AgentFilesystem{
  ...
  private async replaceStaged(target:string,content:string,exclusive:boolean,canCommit:()=>boolean=()=>true):Promise<FileMutation>{
    ...
    try{handle=await fs.open(staged,'wx',0o600);await handle.writeFile(buffer);await handle.sync();await handle.close();handle=undefined;await this.hooks.onWriteStage?.('after-stage');await this.hooks.onWriteStage?.('before-rename');if(!canCommit())throw new OwnershipFenceError('Runner ownership lost before filesystem commit');
  ...
  async create(agentId:string,requested:string,content:string,space:'private'|'shared'='private',canCommit:()=>boolean=()=>true):Promise<FileMutation>{const target=await this.resolve(agentId,requested,space,true);return this.replaceStaged(target,content,true,canCommit);}
  async write(agentId:string,requested:string,content:string,space:'private'|'shared'='private',expectedHash?:string,canCommit:()=>boolean=()=>true):Promise<FileMutation>{const target=await this.resolve(agentId,requested,space,true),prior=await this.current(target);if(space==='shared'&&prior&&expectedHash!==prior.hash)throw new ConcurrentWriteError('Shared file changed; expectedHash is required and must match');return this.replaceStaged(target,content,false,canCommit);}
  async append(agentId:string,requested:string,content:string,space:'private'|'shared'='private',expectedHash?:string,canCommit:()=>boolean=()=>true):Promise<FileMutation>{const target=await this.resolve(agentId,requested,space,true),prior=await this.current(target);if(space==='shared'&&prior&&expectedHash!==prior.hash)throw new ConcurrentWriteError('Shared file changed; expectedHash is required and must match');return this.replaceStaged(target,(prior?.content??'')+content,false,canCommit);}
